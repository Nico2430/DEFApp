...
function LoginScreen() {
  ...
  return <main className="min-h-screen bg-[#fff4b8] text-[#2a1208] p-6 flex items-center justify-center">
    <form onSubmit={login} className="w-full max-w-md rounded-2xl bg-white/95 p-6 shadow-sm border border-[#f2c94c] space-y-4">
      ...
      <Input type="password" required placeholder="Contraseña" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} />
...
async function imprimirRemitoTransito(pedido) {
  try {
    ...
    const nota = pedido.nota ? `<div class="marca"><b>DESCRIPCIÓN:</b> ${escapeHtml(pedido.nota).replaceAll("\n", "<br/>")}</div>` : "";
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Remito en Tránsito</title><style>@page{size:A4 portrait;margin:10mm}body{font-family:Arial,sans-serif;color:#111;font-size:11px}.remito{width:190mm;min-height:270mm;margin:0 auto;border:1px solid #222;padding:7mm;position:relative}.top{display:grid;grid-template-columns:32mm 1fr 58mm;gap:6mm;align-items:start}.logo{width:30mm;object-fit:contain}.empresa{font-size:11px;line-height:1.25}.numero{border:1px solid #222;padding:4mm;text-align:center;font-weight:bold;font-size:14px}.legal{text-align:center;font-weight:bold;margin:4mm 0;border-top:1px solid #222;border-bottom:1px solid #222;padding:2mm 0}.cliente{display:grid;grid-template-columns:1fr 1fr;gap:2mm 10mm;border:1px solid #222;padding:3mm;margin-top:3mm}.cliente div{min-height:5mm}.venta{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;border:1px solid #222;border-top:none}.venta div{padding:2mm;border-right:1px solid #222}.venta div:last-child{border-right:none}table{width:100%;border-collapse:collapse;margin-top:4mm;font-size:11px}th,td{border:1px solid #222;padding:2mm;text-align:center;vertical-align:middle}th{font-weight:bold}.firmas{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:3mm;position:absolute;left:7mm;right:7mm;bottom:14mm}.firmas div{border-top:1px solid #222;text-align:center;padding-top:2mm;font-size:10px}.marca{font-weight:bold;margin-top:4mm}.no-factura{text-align:center;font-size:16px;font-weight:bold;margin-top:5mm}.transito{font-size:18px;font-weight:bold;text-align:center;margin-top:2mm}.x{font-size:22px;font-weight:bold;margin-right:5mm}</style></head><body><section class="remito"><div class="top"><img class="logo" src="${assetUrl(EMDABER_LOGO_SRC)}"/><div class="empresa"><b>Emilio Castro 2543</b><br/>(B1824MOG) – Lanús Oeste<br/>Pcia. De Bs. As. – Argentina<br/>Wsp: 11 2484-7882<br/>Email: aberturas@emdaber.com</div><div class="numero">Nº 0001 – ${String(remito.nro_remito).padStart(8,"0")}</div></div><div class="legal">C.U.I.T.: 30-69417030-3 – ING. BRUTOS: Exento<br/>IMP. INTERNOS: No Responsable – INICIO ACT: 12/03/1998</div><div class="cliente"><div><b>FECHA:</b> ${formatDate(remito.fecha)}</div><div><b>Nº CLIENTE:</b> ${escapeHtml(cliente.nro_cliente || "")}</div><div><b>SEÑOR (ES):</b> ${escapeHtml(cliente.razon_social || "")}</div><div><b>C.U.I.T.:</b> ${escapeHtml(cliente.cuit_cuil || "")}</div><div><b>DOMICILIO:</b> ${escapeHtml(cliente.direccion || "")}</div><div><b>TELEFONO:</b> ${escapeHtml(cliente.telefono || "")}</div><div><b>LOCALIDAD:</b> ${escapeHtml(cliente.localidad || "")}</div><div><b>I.V.A.:</b></div><div><b>PROVINCIA:</b></div></div><div class="venta"><div><b>ORDEN Nº</b><br/>${pedido.id}</div><div><b>PEDIDO</b><br/>${pedido.id}</div><div><b>VTO. FACTURA</b></div><div><b>CONDICIONES DE VENTA</b></div></div><table><thead><tr><th style="width:20mm">CANTIDAD</th><th style="width:38mm">MEDIDA</th><th>ARTÍCULO</th></tr></thead><tbody>${filas}</tbody></table>${nota}<div class="transito"><span class="x">X</span>MERCADERIA EN TRANSITO</div><div class="no-factura">DOCUMENTO NO VÁLIDO COMO FACTURA</div><div class="firmas"><div>CONFECCIONÓ</div><div>REVISÓ</div><div>RECIBÍ CONFORME<br/>ACLARACIÓN DE FIRMA</div><div>Nº DE DOCUMENTO<br/>CARGO QUE DESEMPEÑA</div></div></section><script>window.onload=()=>setTimeout(()=>window.print(),400)</script></body></html>`;
    const htmlFinal = html
      ...
      .replace("</div></div><div class=\"legal\">", "</div><div class=\"header-aviso\"><div class=\"transito\"><span class=\"x\">X</span>MERCADERIA EN TRANSITO</div><div class=\"no-factura\">DOCUMENTO NO VÁLIDO COMO FACTURA</div></div></div><div class=\"legal\">");
    const w = window.open("", "_blank", "width=900,height=1000");
...
export default function DEFApp() {
  ...
  if (authLoading) {
    return <main className="min-h-screen bg-[#fff4b8] text-[#2a1208] p-6 flex items-center justify-center">Cargando sesión...</main>;
  }
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between rounded-3xl border border-[#f2c94c] bg-gradient-to-r from-[#ffe45c] to-[#f6c842] p-5 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-4xl font-bold text-[#4b1f0e]">DEFApp</h1><p className="text-[#6a2b12]">Gestión integral de aberturas de madera</p></div>
        </div>
...
function Clientes({ clientes, reload }) {
  ...
  return <Section title="Clientes"><form onSubmit={save} className="grid md:grid-cols-3 gap-3 mb-5"><Input required placeholder="Razón Social" value={form.razon_social} onChange={e => setForm({ ...form, razon_social: e.target.value })} /><Input required placeholder="Dirección" value={form.direccion} onChange={e => setForm({ ...form, direccion: e.target.value })} /><Input required placeholder="Localidad" value={form.localidad} onChange={e => setForm({ ...form, localidad: e.target.value })} /><Input required placeholder="Teléfono" value={form.telefono} onChange={e => setForm({ ...form, telefono: e.target.value })} /><Input placeholder="CUIT/CUIL opcional" value={form.cuit_cuil || ""} onChange={e => setForm({ ...form, cuit_cuil: e.target.value })} /><Button>{edit ? "Guardar cambios" : "Crear cliente"}</Button></form><Input placeholder="Filtrar por nombre o N° cliente" value={q} onChange={e => setQ(e.target.value)} /><Table rows={rows} cols={["nro_cliente", "razon_social", "direccion", "localidad", "telefono", "cuit_cuil"]} onEdit={r => { setEdit(r.id); setForm({ razon_social: r.razon_social, direccion: r.direccion, localidad: r.localidad, telefono: r.telefono, cuit_cuil: r.cuit_cuil || "" }); }} onDelete={del} /></Section>;
}
...
function Pedidos({ clientes, stock, pedidos, reload }) {
  ...
  async function save() {
    try {
      if (!clienteId || items.length === 0) return alert("Elegí cliente y agregá al menos una abertura.");
...
function Saldos({ saldos, pagos }) {
  ...
  return <Section title="Consulta de saldos"><Input placeholder="Filtrar cliente" value={q} onChange={e => setQ(e.target.value)} /><div className="overflow-auto mt-4"><table className="w-full text-sm"><thead><tr className="text-left border-b"><th>Cliente</th><th>Pedido</th><th>Fecha</th><th>Total</th><th>Pagado</th><th>Saldo</th><th>Pagos</th></tr></thead><tbody>{list.map(s => <tr key={s.pedido_id} className="border-b align-top"><td>#{s.nro_cliente} · {s.razon_social}</td><td>#{s.pedido_id}</td><td>{formatDate(s.fecha)}</td><td>{money(s.total)}</td><td>{money(s.pagado)}</td><td className="font-bold">{money(s.saldo)}</td><td>{pagos.filter(p => p.pedido_id === s.pedido_id).map(p => <div key={p.id}>{formatDate(p.fecha)} · {money(p.monto)} · {p.metodo}</div>)}</td></tr>)}</tbody></table></div></Section>;
}
