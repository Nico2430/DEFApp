...
function LoginScreen() {
  ...
  return <main className="min-h-screen bg-[#fff4b8] text-[#2a1208] p-6 flex items-center justify-center">
    <form onSubmit={login} className="w-full max-w-md rounded-2xl bg-white/95 p-6 shadow-sm border border-[#f2c94c] space-y-4">
      ...
      <Input type="password" required placeholder="ContraseÃ±a" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} />
...
async function imprimirRemitoTransito(pedido) {
  try {
    ...
    const nota = pedido.nota ? `<div class="marca"><b>DESCRIPCIÃ“N:</b> ${escapeHtml(pedido.nota).replaceAll("\n", "<br/>")}</div>` : "";
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Remito en TrÃ¡nsito</title><style>@page{size:A4 portrait;margin:10mm}body{font-family:Arial,sans-serif;color:#111;font-size:11px}.remito{width:190mm;min-height:270mm;margin:0 auto;border:1px solid #222;padding:7mm;position:relative}.top{display:grid;grid-template-columns:32mm 1fr 58mm;gap:6mm;align-items:start}.logo{width:30mm;object-fit:contain}.empresa{font-size:11px;line-height:1.25}.numero{border:1px solid #222;padding:4mm;text-align:center;font-weight:bold;font-size:14px}.legal{text-align:center;font-weight:bold;margin:4mm 0;border-top:1px solid #222;border-bottom:1px solid #222;padding:2mm 0}.cliente{display:grid;grid-template-columns:1fr 1fr;gap:2mm 10mm;border:1px solid #222;padding:3mm;margin-top:3mm}.cliente div{min-height:5mm}.venta{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;border:1px solid #222;border-top:none}.venta div{padding:2mm;border-right:1px solid #222}.venta div:last-child{border-right:none}table{width:100%;border-collapse:collapse;margin-top:4mm;font-size:11px}th,td{border:1px solid #222;padding:2mm;text-align:center;vertical-align:middle}th{font-weight:bold}.firmas{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:3mm;position:absolute;left:7mm;right:7mm;bottom:14mm}.firmas div{border-top:1px solid #222;text-align:center;padding-top:2mm;font-size:10px}.marca{font-weight:bold;margin-top:4mm}.no-factura{text-align:center;font-size:16px;font-weight:bold;margin-top:5mm}.transito{font-size:18px;font-weight:bold;text-align:center;margin-top:2mm}.x{font-size:22px;font-weight:bold;margin-right:5mm}</style></head><body><section class="remito"><div class="top"><img class="logo" src="${assetUrl(EMDABER_LOGO_SRC)}"/><div class="empresa"><b>Emilio Castro 2543</b><br/>(B1824MOG) â€“ LanÃºs Oeste<br/>Pcia. De Bs. As. â€“ Argentina<br/>Wsp: 11 2484-7882<br/>Email: aberturas@emdaber.com</div><div class="numero">NÂº 0001 â€“ ${String(remito.nro_remito).padStart(8,"0")}</div></div><div class="legal">C.U.I.T.: 30-69417030-3 â€“ ING. BRUTOS: Exento<br/>IMP. INTERNOS: No Responsable â€“ INICIO ACT: 12/03/1998</div><div class="cliente"><div><b>FECHA:</b> ${formatDate(remito.fecha)}</div><div><b>NÂº CLIENTE:</b> ${escapeHtml(cliente.nro_cliente || "")}</div><div><b>SEÃ‘OR (ES):</b> ${escapeHtml(cliente.razon_social || "")}</div><div><b>C.U.I.T.:</b> ${escapeHtml(cliente.cuit_cuil || "")}</div><div><b>DOMICILIO:</b> ${escapeHtml(cliente.direccion || "")}</div><div><b>TELEFONO:</b> ${escapeHtml(cliente.telefono || "")}</div><div><b>LOCALIDAD:</b> ${escapeHtml(cliente.localidad || "")}</div><div><b>I.V.A.:</b></div><div><b>PROVINCIA:</b></div></div><div class="venta"><div><b>ORDEN NÂº</b><br/>${pedido.id}</div><div><b>PEDIDO</b><br/>${pedido.id}</div><div><b>VTO. FACTURA</b></div><div><b>CONDICIONES DE VENTA</b></div></div><table><thead><tr><th style="width:20mm">CANTIDAD</th><th style="width:38mm">MEDIDA</th><th>ARTÃCULO</th></tr></thead><tbody>${filas}</tbody></table>${nota}<div class="transito"><span class="x">X</span>MERCADERIA EN TRANSITO</div><div class="no-factura">DOCUMENTO NO VÃLIDO COMO FACTURA</div><div class="firmas"><div>CONFECCIONÃ“</div><div>REVISÃ“</div><div>RECIBÃ CONFORME<br/>ACLARACIÃ“N DE FIRMA</div><div>NÂº DE DOCUMENTO<br/>CARGO QUE DESEMPEÃ‘A</div></div></section><script>window.onload=()=>setTimeout(()=>window.print(),400)</script></body></html>`;
    const htmlFinal = html
      ...
      .replace("</div></div><div class=\"legal\">", "</div><div class=\"header-aviso\"><div class=\"transito\"><span class=\"x\">X</span>MERCADERIA EN TRANSITO</div><div class=\"no-factura\">DOCUMENTO NO VÃLIDO COMO FACTURA</div></div></div><div class=\"legal\">");
    const w = window.open("", "_blank", "width=900,height=1000");
...
export default function DEFApp() {
  ...
  if (authLoading) {
    return <main className="min-h-screen bg-[#fff4b8] text-[#2a1208] p-6 flex items-center justify-center">Cargando sesiÃ³n...</main>;
  }
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between rounded-3xl border border-[#f2c94c] bg-gradient-to-r from-[#ffe45c] to-[#f6c842] p-5 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-4xl font-bold text-[#4b1f0e]">DEFApp</h1><p className="text-[#6a2b12]">GestiÃ³n integral de aberturas de madera</p></div>
        </div>
...
function Clientes({ clientes, reload }) {
  ...
  async function del(id) { if (confirm("Â¿Eliminar cliente?")) { const r = await supabase.from("clientes").delete().eq("id", id); if (r.error) alert(r.error.message); reload(); } }
  return <Section title="Clientes"><form onSubmit={save} className="grid md:grid-cols-3 gap-3 mb-5"><Input required placeholder="RazÃ³n Social" value={form.razon_social} onChange={e => setForm({ ...form, razon_social: e.target.value })} /><Input required placeholder="DirecciÃ³n" value={form.direccion} onChange={e => setForm({ ...form, direccion: e.target.value })} /><Input required placeholder="Localidad" value={form.localidad} onChange={e => setForm({ ...form, localidad: e.target.value })} /><Input required placeholder="TelÃ©fono" value={form.telefono} onChange={e => setForm({ ...form, telefono: e.target.value })} /><Input placeholder="CUIT/CUIL opcional" value={form.cuit_cuil || ""} onChange={e => setForm({ ...form, cuit_cuil: e.target.value })} /><Button>{edit ? "Guardar cambios" : "Crear cliente"}</Button></form><Input placeholder="Filtrar por nombre o NÂ° cliente" value={q} onChange={e => setQ(e.target.value)} /><Table rows={rows} cols={["nro_cliente", "razon_social", "direccion", "localidad", "telefono", "cuit_cuil"]} onEdit={r => { setEdit(r.id); setForm({ razon_social: r.razon_social, direccion: r.direccion, localidad: r.localidad, telefono: r.telefono, cuit_cuil: r.cuit_cuil || "" }); }} onDelete={del} /></Section>;
}
...
function Stock({ stock, reload }) {
  ...
  async function del(id) { if (confirm("Â¿Eliminar abertura?")) { const r = await supabase.from("stock_aberturas").delete().eq("id", id); if (r.error) alert(r.error.message); reload(); } }
...
function Pedidos({ clientes, stock, pedidos, reload }) {
  ...
  async function save() {
    try {
      if (!clienteId || items.length === 0) return alert("ElegÃ­ cliente y agregÃ¡ al menos una abertura.");
  ...
  async function del(id) { if (confirm("Â¿Eliminar pedido? TambiÃ©n se eliminan sus Ã­tems y pagos asociados.")) { const r = await supabase.from("pedidos").delete().eq("id", id); if (r.error) alert(r.error.message); reload(); } }
  const rows = pedidos.map(p => ({ ...p, cliente: p.clientes?.razon_social, archivo: p.archivo_url ? (p.archivo_nombre || "Ver archivo") : "" }));
  return <Section title="Pedidos"><div className="grid md:grid-cols-4 gap-3 mb-4"><Input placeholder="Filtrar cliente por nombre o NÂ°" value={q} onChange={e => setQ(e.target.value)} /><Select value={sort} onChange={e => setSort(e.target.value)}><option value="nro_cliente">Ordenar por NÂ°</option><option value="razon_social">Ordenar por RazÃ³n Social</option></Select><Select value={clienteId} onChange={e => setClienteId(e.target.value)}><option value="">Seleccionar cliente</option>{clientesFiltrados.map(c => <option key={c.id} value={c.id}>#{c.nro_cliente} Â· {c.razon_social}</option>)}</Select><Input type="date" value={fecha} onChange={e => setFecha(e.target.value)} /></div><div className="grid md:grid-cols-5 gap-3 mb-4"><Select value={aberturaId} onChange={e => setAberturaId(e.target.value)}><option value="">Seleccionar abertura</option>{stock.map(a => <option key={a.id} value={a.id}>#{a.nro_abertura} Â· {a.modelo} Â· {a.medida} Â· {a.madera} Â· {a.mano}</option>)}</Select><Input type="number" min="1" value={cantidad} onChange={e => setCantidad(e.target.value)} /><Input type="number" min="0" step="0.01" placeholder="Precio unitario" value={precio} onChange={e => setPrecio(e.target.value)} /><Button type="button" onClick={addItem}>Agregar modelo</Button><Input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={e => setArchivo(e.target.files?.[0] || null)} /></div><div className="mb-4"><TextArea rows="3" placeholder="Texto adicional del pedido. Ej: Tiene barral de 60cms" value={nota} onChange={e => setNota(e.target.value)} /></div><div className="rounded-xl bg-[#fff9d6] border border-[#f2c94c] p-4 mb-5"><h3 className="font-semibold mb-2">Pre-finalizaciÃ³n {editId ? `(editando pedido #${editId})` : ""}</h3>{items.length === 0 ? <p className="text-slate-500">TodavÃ­a no agregaste aberturas.</p> : items.map((i, idx) => <div key={idx} className="flex justify-between border-b py-2"><span>{i.cantidad} x #{i.abertura?.nro_abertura} {i.abertura?.modelo} - {i.abertura?.mano} - {i.abertura?.madera} Â· {i.abertura?.medida}</span><span>{money(i.cantidad * i.precio_unitario)} <button className="ml-2 text-red-600" onClick={() => setItems(items.filter((_, j) => j !== idx))}>Quitar</button></span></div>)}<div className="text-right text-2xl font-bold mt-3">Total: {money(total)}</div><div className="flex justify-end gap-2 mt-3"><Button type="button" onClick={save} disabled={!clienteId || items.length === 0}>{editId ? "Guardar e imprimir" : "Finalizar e imprimir"}</Button>{editId && <Button type="button" className="bg-slate-500" onClick={reset}>Cancelar ediciÃ³n</Button>}</div></div><Table rows={rows} cols={["id", "fecha", "cliente", "total", "archivo"]} onEdit={editPedido} onDelete={del} extra={(r) => <>{r.archivo_url && <button className="text-emerald-700" onClick={() => verAdjunto(r.archivo_url, r.archivo_nombre)}>Ver adjunto</button>}<button className="text-[#4b1f0e]" onClick={() => imprimirPedido({ pedidoId: r.id, cliente: r.clientes, fecha: r.fecha, items: (r.pedido_items || []).map(i => ({ abertura: i.stock_aberturas, cantidad: i.cantidad, precio_unitario: Number(i.precio_unitario) })), total: r.total, nota: r.nota })}>Imprimir pedido</button><button className="text-[#8a5a00]" onClick={() => imprimirRemitoTransito(r)}>Imprimir remito</button></>} /></Section>;
}
...
function Movimientos({ movimientos, proveedores, reload }) {
  ...
  async function del(id) { if (confirm("Â¿Eliminar movimiento?")) { await supabase.from("movimientos").delete().eq("id", id); reload(); } }
  return <Section title="Movimientos / Facturas"><form onSubmit={save} className="grid md:grid-cols-4 gap-3 mb-5"><input list="proveedores" required placeholder="RazÃ³n Social" value={form.razon_social} onChange={e => proveedorChange(e.target.value)} className="w-full rounded-xl border border-[#e6c65a] px-3 py-2" /><datalist id="proveedores">{proveedores.map(p => <option key={p.id} value={p.razon_social} />)}</datalist><Input placeholder="CUIT/CUIL" value={form.cuit_cuil || ""} onChange={e => setForm({ ...form, cuit_cuil: e.target.value })} /><Input placeholder="Rubro" value={form.rubro || ""} onChange={e => setForm({ ...form, rubro: e.target.value })} /><Input type="date" required value={form.fecha} onChange={e => setForm({ ...form, fecha: e.target.value })} /><Input required placeholder="NÂ° Factura" value={form.nro_factura} onChange={e => setForm({ ...form, nro_factura: e.target.value })} /><Input type="number" step="0.01" min="0" required placeholder="Importe" value={form.importe} onChange={e => setForm({ ...form, importe: e.target.value })} /><Button>Cargar factura</Button></form><Table rows={movimientos} cols={["fecha", "razon_social", "cuit_cuil", "rubro", "nro_factura", "importe"]} onDelete={del} /></Section>;
}
